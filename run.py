...
def train_coco(model):
    train_set = TrainCOCO('../DB/COCO', clip_l=5, clip_n=128)
    ...
    # trainer.load_checkpoint(save_name='coco', epoch=8000, device='cuda:0')
    trainer.train(8000)


def train_davis(model):
    train_set = TrainDAVIS('../DB/DAVIS', '2017', 'train', clip_l=10, clip_n=128)
...
def train_ytvos(model):
    train_set = TrainYTVOS('../DB/YTVOS18', 'cho_train', clip_l=10, clip_n=128)
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=8, num_workers=4, pin_memory=True)
    val_set = TestYTVOS('../DB/YTVOS18', 'cho_val')
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
